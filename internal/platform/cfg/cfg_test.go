...
// TestExists validates the ability to load/retrieve configuration values
// using environment variables
func TestExists(t *testing.T) {
	t.Log("Given the need to read environment variable")
	{
		uStr := "mongodb://127.0.0.1:3000"
		dStr := 2 * time.Minute
		...
		t.Log("\tWhen given a namespace key to search for that exists")
		{
			...
			if err != nil {
				t.Errorf("\t\t%s Should not return error when valid key %s", failed, "PROC_ID")
			} else {
				t.Logf("\t\t%s Should not return error when valid key %s", success, "PROC_ID")
			...
			if err != nil {
				t.Errorf("\t\t%s Should not return error when valid key %q", failed, "SCOKET")
			} else {
...
// TestNotExists validates the ability to load configuration using environment varaibles
// and panic when some thing is missing
func TestNotExists(t *testing.T) {
	t.Log("Given the need to panic when environment variables are missing")
	{
		...
		t.Log("\tWhen given a namespace key to search for that does not exist")
		{
...
// shouldNotPanic receive a ctx string and a function to run
// panic is a failure
func shouldNotPanic(t *testing.T, ctx string, fx func()) {
	defer func() {
		t.Helper()
		if err := recover(); err == nil {
			t.Logf("\t\t%s SHould not have pacicked when giving known key %q.", success, ctx)
		} else {
			t.Errorf("\t\t%s Should not have panicked when giving known key %q.", failed, ctx)
		}
	}()
...
// shouldPanic receive a ctx string and a function to run
// panic is a success
...
func TestSets(t *testing.T) {
	t.Log("Given the need to manually set configuration values")
	{
		...
		t.Logf("\tWhen setting values")
		{
			...
			key = "key5"
			urlVal, _ := url.Parse("mongodb://127.0.0.1:3000/gotraining?sort=1")
